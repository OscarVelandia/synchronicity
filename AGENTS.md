...
- Do not derive behavior-named Redux values inside components, especially `canX`, `shouldX`, or enabled/disabled conditions. Put same-slice derivations in the slice's `selectors` option and cross-slice derivations in a focused selector module, then consume them point-free from components.
